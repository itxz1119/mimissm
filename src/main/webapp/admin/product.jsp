...
<script type="text/javascript">
    ...
    function deleteBatch(page) {
        ...
        if(zhi.length==0){
            ...
            if (confirm("您确定删除" + zhi.length + "条商品吗?")){
                ...
                //alert(str + "  1111");
                <%--window.location = "${pageContext.request.contextPath}/prod/deleteByIds.action?str=" + str;--%>
                $.ajax({
                    url:"${pageContext.request.contextPath}/prod/deleteByIds.action",
    ...
    function one(pid, ispage) {
        ...
        var lprice = $("#lprice").val();
        var hprice = $("#hprice").val();
        var str = "&page=" + ispage + "&pname=" + pname +  "&typeid=" + typeid +"&lprice=" +lprice + "&hprice=" + hprice
